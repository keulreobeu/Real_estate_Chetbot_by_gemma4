...
이 프로젝트에서 저는 Gemma 4 모델 자체를 새로 개발한 것이 아니라, 부동산 챗봇 문제를 데이터 계약, 검색 기반 응답, 평가 기준, 안전 규칙, 파인튜닝 준비 단계로 구조화하는 역할을 주도했습니다.

원천 아파트 CSV를 RAG용 데이터셋, QA/평가 데이터셋, 엣지 케이스 검증 세트, SFT 후보/holdout/train-valid JSONL로 이어지게 만들고, 각 단계의 입력·출력·검증 기준을 문서와 보고서로 관리했습니다.

특히 추천형 질문에서 근거 부족이나 조건 불일치가 있을 때 임의 아파트를 추천하지 않도록 `NO_MATCH`, `UNKNOWN`, `must_not_recommend`, `must_disclose_limit` 기준을 두고, 라우팅 실패와 추천 안전성 실패를 별도로 분석하는 평가 흐름을 구성했습니다.

이 저장소는 "모델을 직접 만든 프로젝트"라기보다, AI 도구를 활용해 부동산 챗봇 문제를 실행 가능한 데이터 파이프라인과 평가 가능한 서비스 구조로 구체화한 작업에 가깝습니다.
